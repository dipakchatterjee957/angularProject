...
import { from, fromEvent, interval, map, take, takeLast, takeUntil, takeWhile, tap, toArray } from 'rxjs';
...
export class TakeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // the takeUntil function condition false and the insert value operation was stopped.
    ...
    // Ex - 4
    const obs4 = from(array);
    obs4.pipe(
      takeWhile((x, index) => index < 4),
    ).subscribe(res => {
      console.log(res);
